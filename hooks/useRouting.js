...
export const useRouting = (location, destination, selectedTerminal, customPaths, terminals, mapRef) => {
    ...
    const getRoute = async () => {
      try {
        ...
        if (activeTerminal.isComboPair && activeTerminal.comboTrainOrig && activeTerminal.comboTrainDest) {
             ...
             const segments = [
               { type: 'walk', coords: [{ latitude: locLat, longitude: locLng }, { latitude: activeTerminal.latitude, longitude: activeTerminal.longitude }] },
               { type: 'ride', coords: pathCoords, isDashed: false, category: activeTerminal.category },
               { type: 'walk', coords: [jeepDropoff, { latitude: trainOrig.latitude, longitude: trainOrig.longitude }] },
             ...
             if (distToDest > 1.0) {
                 ...
                 if (closestTricycle) {
                   ...
                   segments.push({ type: 'walk', coords: [{ latitude: trainDest.latitude, longitude: trainDest.longitude }, { latitude: closestTricycle.latitude, longitude: closestTricycle.longitude }] });
                   try {
             ...
             if (!chainedTricycle) {
                 segments.push({ type: 'walk', coords: [{ latitude: trainDest.latitude, longitude: trainDest.longitude }, { latitude: destLat, longitude: destLng }] });
        ...
        if (activeTerminal.isTrainPair && activeTerminal.trainDest) {
           ...
           allGeometries.push([
             ...
             { type: 'walk', coords: [{ latitude: destStation.latitude, longitude: destStation.longitude }, { latitude: destLat, longitude: destLng }] }
           ]);
          ...
          if (routeKeys.length > 0) {
            ...
            for (let idx = 0; idx < pathsToRender.length; idx++) {
              ...
              if (distToDest > 1.0) {
                ...
                if (closestTricycle) {
                  chainedTricycle = true;
                  segments.push({ type: 'walk', coords: [endPoint, { latitude: closestTricycle.latitude, longitude: closestTricycle.longitude }] });
              ...
              if (!chainedTricycle) {
                segments.push({ type: 'walk', coords: [endPoint, { latitude: destLat, longitude: destLng }] });
        ...
        if (allOptions.length > 1) {
           allOptions.sort((a, b) => (a.sortScore || a.durationMins) - (b.sortScore || b.durationMins));
           allOptions.forEach((opt, idx) => {
              opt.id = `opt_${idx}`;
              if (idx === 0) {
                 opt.badge = 'Fastest';
                 opt.title = opt.title.replace('Alternative', 'Route');
              } else {
                 opt.badge = 'Alt';
                 if (!opt.title.includes('Alt')) opt.title = `Alt: ${opt.title}`;
              }
           });
        }
        ...
        if (allGeometries.length === 0) {
          ...
          if (res.data.routes && res.data.routes.length > 0) {
            res.data.routes.forEach((route, idx) => {
              ...
              if (idx === 0) {
                allOptions = generateMultiModalRouteOptions(parseFloat(distKm), durationMinutes, destination.name ? destination.name : '');
              } else {
                allOptions.push({
                  id: `osrm_${idx}`,
