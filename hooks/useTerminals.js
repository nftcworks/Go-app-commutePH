...
export const useTerminals = () => {
  ...
  const addTerminal = async (newTerminal) => {
    try {
      ...
      if (!userId) {
        Alert.alert("Sign In Required", "You must be signed in to add a terminal.");
        return;
      }
      ...
      // Update UI with the actual database ID to prevent Realtime duplication
      if (data && data.length > 0) {
        setIncidents(prev => {
          const exists = prev.find(i => i.id === data[0].id);
          if (exists) return prev;
          const newIncidents = [...prev, data[0]];
          AsyncStorage.setItem('@terminals_cache', JSON.stringify(newIncidents)).catch(e => console.log(e));
          return newIncidents;
        });
      }
  ...
  const removeTerminal = async (id) => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.user) {
        Alert.alert("Sign In Required", "You must be signed in to delete a terminal.");
        return;
      }
  ...
  const updateTerminal = async (id, updates) => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.user) {
        Alert.alert("Sign In Required", "You must be signed in to update a terminal.");
        return;
      }
      // Optimistic UI update
      setIncidents(prev => {
